...
def FinderShowHiddenFiles(passw):
    ...
    util.ext_call(['defaults', 'write', 'com.apple.finder',
...
def FinderHideExtDisksFromDesktop(passw):
    ...
    util.ext_call(['defaults', 'write', 'com.apple.finder',
                   'ShowExternalHardDrivesOnDesktop', '-int', '0'])

# Description:
# This tweak will show all removable media (CDs, DVDs, iPods etc.)
# on the desktop
...
# Description:
# This tweak will hide all removable media (CDs, DVDs, iPods etc.)
# from the desktop
def FinderHideRemovableMediaFromDesktop(passw):
    print('Hiding removable media from desktop...')
    util.ext_call(['defaults', 'write', 'com.apple.finder',
                   'ShowRemovableMediaOnDesktop', '-int', '0'])
...
def FinderHideConnectedServersFromDesktop(passw):
    ...
    util.ext_call(['defaults', 'write', 'com.apple.finder',
                   'ShowMountedServersOnDesktop', '-int', '0'])
...
def FinderStartupWindowHome(passw):
    ...
    util.ext_call(['defaults', 'write', 'com.apple.finder',
                   'NewWindowTargetPath', 'file://%s' % home])
...
def FinderShowAllFilenameExtensionsOn(passw):
    ...
    util.ext_call(['defaults', 'write', 'NSGlobalDomain',
                   'AppleShowAllExtensions', '-int', '1'])
...
def FinderShowWarningOnExtensionChangeOn(passw):
    ...
    util.ext_call(['defaults', 'write', 'com.apple.finder',
                   'FXEnableExtensionChangeWarning', '-int', '1'])
...
def FinderShowWarningBeforeRmFromICDriveOn(passw):
    ...
    util.ext_call(['defaults', 'write', 'com.apple.finder',
                   'FXEnableRemoveFromICloudDriveWarning', '-int', '1'])
...
def FinderShowWarningBeforeRmFromICDriveOff(passw):
    ...
    util.ext_call(['defaults', 'write', 'com.apple.finder',
                   'FXEnableRemoveFromICloudDriveWarning', '-int', '0'])
...
def DockShowOnlyActiveApp(passw):
    ...
    util.ext_call(['defaults', 'write', 'com.apple.dock',
                   'single-app', '-bool', 'true'])
...
def DockGroupWindowsByApplicationOff(passw):
    ...
    util.ext_call(['defaults', 'write', 'com.apple.dock',
                   'expose-group-apps', '-int', '0'])

# Description:
# This tweak will turn off macOS's dashboard
def DockDashboardOff(passw):
    print('Turning off Dashboard...')
    util.ext_call(['defaults', 'write', 'com.apple.dashboard',
                   'dashboard-enabled-state', '-int', '1'])
...
def SystemAutoHideMenuBarOff(passw):
    ...
    util.ext_call(['defaults', 'write', 'NSGlobalDomain',
                   '_HIHideMenuBar', '-int', '0'])

# Description:
# This tweak will change the color variant of buttons,
# menus and windows to Blue
def SystemAppearanceBlue(passw):
    print('Changing Appearance to blue...')
    util.ext_call(['defaults', 'write', 'NSGlobalDomain',
                   'AppleAquaColorVariant', '-int', '1'])

# Description:
# This tweak will change the color variant of buttons,
# menus and windows to Graphite
def SystemAppearanceGraphite(passw):
    print('Changing Appearance to graphite...')
    util.ext_call(['defaults', 'write', 'NSGlobalDomain',
                   'AppleAquaColorVariant', '-int', '6'])
...
def SystemSidebarIconsLarge(passw):
    ...
    util.ext_call(['defaults', 'write', 'NSGlobalDomain',
                   'NSTableViewDefaultSizeMode', '-int', '3'])
...
def SystemClickScrollbarNextPage(passw):
    ...
    util.ext_call(['defaults', 'write', 'NSGlobalDomain',
                   'AppleScrollerPagingBehavior', '-int', '0'])
...
def SystemClickScrollbarClickedSpot(passw):
    ...
    util.ext_call(['defaults', 'write', 'NSGlobalDomain',
                   'AppleScrollerPagingBehavior', '-int', '1'])
...
def SystemAskToKeepChanges(passw):
    ...
    util.ext_call(['defaults', 'write', 'NSGlobalDomain',
                   'NSCloseAlwaysConfirmsChanges', '-int', '1'])
...
def SystemDontAskToKeepChanges(passw):
    ...
    util.ext_call(['defaults', 'write', 'NSGlobalDomain',
                   'NSCloseAlwaysConfirmsChanges', '-int', '0'])
...
def SystemCloseWindowsOnAppQuit(passw):
    ...
    util.ext_call(['defaults', 'write', 'NSGlobalDomain',
                   'NSQuitAlwaysKeepsWindows', '-int', '0'])
...
def SystemDontCloseWindowsOnAppQuit(passw):
    ...
    util.ext_call(['defaults', 'write', 'NSGlobalDomain',
                   'NSQuitAlwaysKeepsWindows', '-int', '1'])
...
def SystemSpacesSwitchOnActivateOff(passw):
    ...
    util.ext_call(['defaults', 'write', 'NSGlobalDomain',
                   'AppleSpacesSwitchOnActivate', '-int', '0'])
...
def SystemDisplaysToSeparateSpacesOn(passw):
    ...
    util.ext_call(['defaults', 'write', 'com.apple.spaces',
                   'spans-displays', '-int', '1'])
...
def SystemFnKeysMedia(passw):
    ...
    util.ext_call(['defaults', 'write', 'NSGlobalDomain',
                   'com.apple.keyboard.fnState', '-int', '0'])
...
def SystemSilentClickOn(passw):
    ...
    util.ext_call(['defaults', 'write', 'com.apple.AppleMultitouchTrackpad',
                   'ActuationStrength', '-int', '0'])
...
def SystemScrollDirectionNatural(passw):
    ...
    util.ext_call(['defaults', 'write', 'NSGlobalDomain',
                   'com.apple.swipescrolldirection', '-int', '1'])
...
def SystemScrollDirectionReversed(passw):
    ...
    util.ext_call(['defaults', 'write', 'NSGlobalDomain',
                   'com.apple.swipescrolldirection', '-int', '0'])
...
def SystemShortcutsSetLaunchTerminal(passw):
    ...
    util.ext_call(
        [util.PLISTBUDDY, '-c',
         'Delete %s:key_equivalent' % place, plist_path])
    util.ext_call(
        [util.PLISTBUDDY, '-c',
         'Delete %s:enabled_context_menu' % place, plist_path])
    util.ext_call(
        [util.PLISTBUDDY, '-c',
         'Delete %s:enabled_services_menu' % place, plist_path])
    util.ext_call([util.PLISTBUDDY, '-c', 'Add %s:key_equivalent string %s' %
                   (place, key_combo), plist_path])
...
def SystemUIShowBatteryStatusIndicator(passw):
    ...
    util.ext_call(['defaults', 'write', 'com.apple.systemuiserver',
                   'NSStatusItem Visible com.apple.menuextra.battery',
                   '-int', '1'])
...
def SystemUIShowBatteryPercentage(passw):
    ...
    util.ext_call(['defaults', 'write', 'com.apple.menuextra.battery',
                   'ShowPercent', 'YES'])
...
def SystemUIShowWifiStatusIndicator(passw):
    ...
    util.ext_call(['defaults', 'write', 'com.apple.systemuiserver',
                   'NSStatusItem Visible com.apple.menuextra.airport',
                   '-int', '1'])
...
def SystemUIShowBluetoothStatusIndicator(passw):
    ...
    util.ext_call(['defaults', 'write', 'com.apple.systemuiserver',
                   'NSStatusItem Visible com.apple.menuextra.bluetooth',
                   '-int', '1'])
...
def SystemUIShowClock(passw):
    ...
    util.ext_call(['defaults', 'write', 'com.apple.systemuiserver',
                   'NSStatusItem Visible com.apple.menuextra.clock',
                   '-int', '1'])
...
def SystemUIClockAnalog(passw):
    ...
    util.ext_call(['defaults', 'write', 'com.apple.menuextra.clock',
                   'IsAnalog', '-int', '1'])
...
def SystemUIShowAccessibilityIndicator(passw):
    ...
    util.ext_call(['defaults', 'write', 'com.apple.systemuiserver',
                   'NSStatusItem Visible com.apple.menuextra.universalaccess',
                   '-int', '1'])
...
def TerminalSetStartupPredefProfileBasic(passw):
    ...
    util.ext_call(['defaults', 'write', 'com.apple.Terminal',
...
def TerminalSetNewWindowSameProfile(passw):
    ...
    util.ext_call(['defaults', 'write', 'com.apple.Terminal',
                   'NewWindowSettingsBehavior', '-int', '2'])
...
def TerminalSetNewWindowDefaultProfile(passw):
    ...
    util.ext_call(['defaults', 'write', 'com.apple.Terminal',
                   'NewWindowSettingsBehavior', '-int', '1'])
...
def TerminalSetNewWindowDefaultWorkDir(passw):
    ...
    util.ext_call(['defaults', 'write', 'com.apple.Terminal',
                   'NewWindowWorkingDirectoryBehavior', '-int', '1'])
...
def TerminalSetNewTabSameWorkDir(passw):
    ...
    util.ext_call(['defaults', 'write', 'com.apple.Terminal',
                   'NewTabWorkingDirectoryBehavior', '-int', '2'])
...
def TerminalSetNewTabDefaultWorkDir(passw):
    ...
    util.ext_call(['defaults', 'write', 'com.apple.Terminal',
                   'NewTabWorkingDirectoryBehavior', '-int', '1'])
...
def TerminalSwitchTabKeysOn(passw):
    ...
    util.ext_call(['defaults', 'write', 'com.apple.Terminal',
                   'Command1Through9SwitchesTabs', '-int', '1'])
...
def TerminalSwitchTabKeysOff(passw):
    ...
    util.ext_call(['defaults', 'write', 'com.apple.Terminal',
                   'Command1Through9SwitchesTabs', '-int', '0'])
